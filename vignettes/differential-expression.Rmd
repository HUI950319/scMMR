...
scMMR provides a streamlined workflow for differential expression (DE) analysis
across cell types, followed by GSEA enrichment and correlation analysis with
embedding features.

## Step 1: Differential Expression (RunDE)

`RunDE()` wraps Seurat's `FindMarkers` to run DE across all cell types between
two conditions:
...
Shows top genes per cell type with size = -log10(p-value) and color = logFC:
...
Correlate embedding dimensions with cell type proportions or module scores:
...
- `RunGsea` requires at least ~50 DE genes per cell type for meaningful results
